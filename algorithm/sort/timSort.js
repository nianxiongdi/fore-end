...
/*
合并从 baseA 开始的 lengthA 元素和 lengthB 元素
以稳定的方式就地从 baseB 开始。 lengthA 和 lengthB 必须 > 0，
且 baseA + lengthA == baseB。
还必须有 array[baseB] < array[baseA]，array[baseA + lengthA - 1] 属于合并的末尾，
并且应该有 lengthA <= lengthB。
*/
...
function MergeLow(sortState, baseA, lengthAArg, baseB, lengthBArg) {
  ...
  while (1) {
    ...
    let nofWinsA = 0; // A 连续获胜的次数。
    let nofWinsB = 0; // B 连续获胜的次数。
...
// array[base+offset-1] <=key<array[base+offset]
//hint标记从分区中的哪里开始搜索
// https://cloud.tencent.com/developer/article/1125476
